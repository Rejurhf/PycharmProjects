...
class Flow2d:
    ...
    def calcUDecrease(self, u, v):
        ...
        while yHeight - 2*iter > 0:
            ...
            for i in range(int(yHeight/2)-1, iter-1, -1):
                ...
                count += 1
                # print(minXPos-iter-1, i, tmpMul, minYPos+i, maxYPos-i)
            iter += 1
            mul *= 0.5
        ...
        while yHeight - 2*iter > 0:
            ...
            for i in range(int(yHeight/2)-1, iter-1, -1):
                ...
                for j in range(0, count):
                    tmpMul *= 0.6
                u[minYPos+i, minXPos+(2*iter)+1] = u[minYPos+i, minXPos+(2*iter)+1] * (1-tmpMul)
                u[maxYPos-i, minXPos+(2*iter)+1] = u[maxYPos-i, minXPos+(2*iter)+1] * (1-tmpMul)
                u[minYPos+i, minXPos+(2*iter)+2] = u[minYPos+i, minXPos+(2*iter)+2] * (1-tmpMul*0.7)
                u[maxYPos-i, minXPos+(2*iter)+2] = u[maxYPos-i, minXPos+(2*iter)+2] * (1-tmpMul*0.7)
                count += 1
    ...
    def showPlot(self, X, Y, u, v, p, obstacle, titleText="no text"):
        ...
        plt.contour(X, Y, p)
        M = np.hypot(u, v)
        plt.quiver(X, Y, u, v, M)  ##plotting velocity
